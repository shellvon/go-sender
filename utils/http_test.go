...
func testThrowHTTPErrorsCase(
	t *testing.T,
	url, path string,
	opts utils.HTTPRequestOptions,
	wantCode int,
	wantBody string,
	wantErr bool,
) {
	body, code, err := utils.DoRequest(context.Background(), url+path, opts)
	if wantErr && err == nil {
		t.Errorf("expected error, got nil, code=%d, body=%s", code, body)
	}
	if !wantErr && err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	if code != wantCode {
		t.Errorf("expected code %d, got %d", wantCode, code)
	}
...
func TestDoRequest_ThrowHTTPErrors(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/ok":
			w.WriteHeader(http.StatusOK)
	...
	t.Run("2xx no throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(t, ts.URL, "/ok", utils.HTTPRequestOptions{ThrowHTTPErrors: false}, 200, "ok", false)
	})
	t.Run("2xx throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(t, ts.URL, "/ok", utils.HTTPRequestOptions{ThrowHTTPErrors: true}, 200, "ok", false)
	})
	t.Run("3xx no throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(
			t,
			ts.URL,
			"/redir",
			utils.HTTPRequestOptions{ThrowHTTPErrors: false},
			302,
			"redirect",
			false,
		)
	})
	t.Run("3xx throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(
			t,
			ts.URL,
			"/redir",
			utils.HTTPRequestOptions{ThrowHTTPErrors: true},
			302,
			"redirect",
			false,
		)
	})
	t.Run("4xx no throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(
			t,
			ts.URL,
			"/bad",
	...
	t.Run("4xx throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(
			t,
			ts.URL,
			"/bad",
	...
	t.Run("5xx no throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(
			t,
			ts.URL,
			"/fail",
	...
	t.Run("5xx throw", func(t *testing.T) {
		testThrowHTTPErrorsCase(
			t,
			ts.URL,
			"/fail",
