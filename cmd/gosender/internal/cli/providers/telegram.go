...
func createTelegramProvider(accounts []*telegram.Account) (core.Provider, error) {
	if len(accounts) == 0 {
		return nil, fmt.Errorf("no valid telegram accounts found")
	}
...
func buildTelegramMessage(flags *cli.CLIFlags) (core.Message, error) {
	...
	if len(flags.To) == 0 {
		return nil, fmt.Errorf("at least one recipient (chat_id) is required for telegram messages")
	}
	...
	default:
		return nil, fmt.Errorf("unsupported message type: %s for telegram, currently only 'text' is supported", messageType)
	}
}
...
func validateTelegramFlags(flags *cli.CLIFlags) error {
	if len(flags.To) == 0 {
		return fmt.Errorf("telegram requires at least one recipient (chat_id)")
	}

	if flags.Content == "" {
		return fmt.Errorf("telegram requires content")
	}

	if flags.MessageType != "" && flags.MessageType != "text" {
		return fmt.Errorf("invalid message type '%s' for telegram, only 'text' is currently supported", flags.MessageType)
	}
