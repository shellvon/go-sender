...
func (b *AliyunSMSBuilder) newMessage(msgType MessageType, mobiles []string, content string, opts ...MessageOption) *Message {
	// Build base options
...
// 短信模板 Code。必须使用已审核通过的模板 Code 发送短信。
//
...
func WithAliyunCalledShowNumber(showNumber string) MessageOption {
	return WithExtra(aliyunCalledShowNumber, showNumber)
}
...
func WithAliyunPlayTimes(times int) MessageOption {
	return WithExtra(aliyunPlayTimes, strconv.Itoa(times))
}
...
//
// SMS：可选字段，外部流水扩展字段。
// MMS: 可选字段，预留给调用方使用的 ID。
// Voice: 预留给调用方的 ID，最终会通过回执消息将此 ID 带回给调用方。字符串类型，长度限制为 1~15 个字符。
...
// 卡片短信可选参数，回落文本短信的模板 Code。FallbackType 选择 SMS 回落文本短信时，此参数必填
// https://help.aliyun.com/zh/sms/developer-reference/api-dysmsapi-2017-05-25-sendcardsms
...
// 卡片短信可选参数，回落数字短信的模板 Code。FallbackType 选择 DIGITALSMS 回落数字短信时，此参数必填。
// https://help.aliyun.com/zh/sms/developer-reference/api-dysmsapi-2017-05-25-sendcardsms
...
// 卡片短信可选参数: 回落文本短信的模板变量对应的实际值。SmsTemplateCode 回落的文本短信模板内含有变量时，此参数必填。
// https://help.aliyun.com/zh/sms/developer-reference/api-dysmsapi-2017-05-25-sendcardsms
...
// 卡片短信可选参数: 回落数字短信的模板变量对应的实际值。DigitalTemplateCode 回落的数字短信模板内含有变量时，此参数必填。
// https://help.aliyun.com/zh/sms/developer-reference/api-dysmsapi-2017-05-25-sendcardsms
...
// 可选参数:上行短信扩展码。上行短信指发送给通信服务提供商的短信，用于定制某种服务、完成查询，或是办理某种业务等，需要收费，按运营商普通短信资费进行扣费。
...
// https://help.aliyun.com/zh/sms/developer-reference/api-dysmsapi-2017-05-25-sendcardsms
func WithAliyunCardObjects(cardObjects []map[string]interface{}) MessageOption {
